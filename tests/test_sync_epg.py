...
from tools.sync_epg import mappings_for_source, parse_xmltv_time


class XmltvTimeTests(unittest.TestCase):
    def test_positive_offset_converts_to_utc(self):
        self.assertEqual(
            parse_xmltv_time("20260819143000 +0200"),
            datetime(2026, 8, 19, 12, 30, tzinfo=timezone.utc),
        )

    def test_negative_offset_converts_to_utc(self):
        self.assertEqual(
            parse_xmltv_time("20260819143000 -0300"),
...
class MappingTests(unittest.TestCase):
    def test_modern_source_mappings_are_used(self):
        manifest = {
            ...
            "channels": [
                {
                    "canonicalId": "channel-a",
                    "sourceMappings": [
                        {"sourceId": "source-a", "channelId": "Upstream.A", "priority": 1, "enabled": True}
                    ],
                }
            ],
        }
            ...
            "channels": [
                {
                    "canonicalId": "channel-a",
                    "sourceMappings": [
                        {"sourceId": "source-a", "channelId": "Upstream.A", "priority": 1, "enabled": False}
                    ],
                }
            ],
        }
